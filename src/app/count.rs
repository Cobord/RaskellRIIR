use crate::{
    convenience::{from_bool_seq, question_where, sample},
    core::{from_bool, indices_of, max_kqv, BoolSequence, Predicate, Sequence, Token},
    utils::filled_with,
};
...
fn rasp_count(inputs: &Sequence) -> Sequence {
    ...
    let start_counting = idxs
        .iter()
        .zip(last_sos.clone())
        .map(|(a, b)| *a == b + Token(2))
        .collect::<BoolSequence>();
    let count_tos = max_kqv(
        inputs,
        &last_sos.iter().map(|a| *a + Token(2)).collect(),
        &equality,
        inputs,
    );
    let count_froms = max_kqv(
        inputs,
